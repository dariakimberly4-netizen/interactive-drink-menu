...
import { SoftwareRenderer } from './software-renderer.js?v=2';
...
boardRim.renderOrder=1;boardRim.rotation.x=-Math.PI/2;boardRim.position.y=-.08;turntable.add(boardRim);
...
function createPizza(product){
 ...
 body.renderOrder=2;body.castShadow=true;body.receiveShadow=true;group.add(body);
 ...
 crust.renderOrder=4;crust.rotation.x=-Math.PI/2;crust.position.y=.073;crust.castShadow=true;group.add(crust);
 ...
 top.renderOrder=3;top.rotation.x=-Math.PI/2;top.position.y=.102;top.receiveShadow=true;group.add(top);
 return group;
}
...
function updatePizza(product){
 ...
 if(product.id==='duo'){
  ...
  a.scale.setScalar(.52);b.scale.setScalar(.52);a.position.set(-.8,.025,0);b.position.set(.8,.025,0);pizzaGroup.add(a,b);
...
sourceImage.onload=()=>{loaded=true;updatePizza(current);renderer.render(scene,camera)};
...
function updateCamera(){camera.position.set(distance*Math.sin(polar)*Math.sin(azimuth),distance*Math.cos(polar),distance*Math.sin(polar)*Math.cos(azimuth));camera.lookAt(0,0,0);renderer.render(scene,camera)}
...
new ResizeObserver(()=>{const w=stage.clientWidth,h=stage.clientHeight;renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();renderer.render(scene,camera);}).observe(stage);
