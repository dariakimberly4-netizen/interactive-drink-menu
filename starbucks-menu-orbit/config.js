...
(() => {
  ...
  style.textContent = `
    ...
    .customer-footer button:active{transform:translateY(-2px) scale(.98)}
    .customer-footer button.active{color:#fff8e9;background:rgba(255,255,255,.07)}
    .customer-footer button.active::after{content:"";position:absolute;bottom:3px;width:24px;height:2px;border-radius:99px;background:#d4ad62;box-shadow:0 0 10px rgba(212,173,98,.55)}
    .customer-footer .footer-icon{font-size:19px;line-height:1}.stage{bottom:94px!important}
    @media(max-width:520px){.customer-footer{width:calc(100% - 24px);height:64px;bottom:calc(10px + env(safe-area-inset-bottom));padding:6px 8px}.customer-footer button{font-size:10px}.customer-footer .footer-icon{font-size:18px}.stage{bottom:86px!important}}
  `;
  ...
  const installFooter=()=>{
    ...
    footer.innerHTML=`<button type="button" data-footer-action="menu" class="active" aria-label="Menu Orbit"><span class="footer-icon">◉</span><span>Menu</span></button><button type="button" data-footer-action="pickup" aria-label="Pickup"><span class="footer-icon">⌖</span><span>Pickup</span></button><button type="button" data-footer-action="cart" aria-label="Cart"><span class="footer-icon">▣</span><span>Cart</span></button><button type="button" data-footer-action="account" aria-label="My Account"><span class="footer-icon">◌</span><span>Account</span></button>`;
